  ...
  layers: [
    {
      ...
      scenes: [
        ...
        },
        {
          ...
          parts: 8,
          ms: 7000,
          narration: "切成 8 片也一樣：只要每一片都一樣大，還是平均分。重點不是切幾片，而是每一份都要一樣大。",
        },
        {
          kind: "bar",
          parts: 4,
          ms: 7000,
          narration: "換成長條巧克力也可以：平均切成 4 格、每一格一樣大。平均分不挑形狀，圓形、長條都行。",
        },
      ],
      quiz: [
        {
        ...
        {
          id: "ep-q3",
          prompt: "大雄把一個蛋糕切成 4 塊，但其中一塊特別大。這樣是「平均分」嗎？",
          options: ["是，因為剛好有 4 塊", "不是，每一塊要一樣大", "是，因為每人都有得吃"],
          answer: 1,
          explain: "平均分只看「每一份是不是一樣大」，有一塊特別大就不公平，不算平均分。",
          hints: ["回想第一個畫面：一大一小公平嗎？", "平均分的重點是每一份都一樣大。", "有一塊特別大，就不是平均分。"],
          visual: { kind: "uneven" },
        },
      ],
    },
    {
      ...
      scenes: [
        ...
        },
        {
          ...
          showNum: true,
          ms: 7500,
          narration: "換個例子再看一次：這條巧克力一共 6 格，6 寫在下面當分母；塗色 2 格，2 寫在上面當分子。",
        },
        {
          kind: "frac",
          num: 2,
          den: 6,
          ms: 6500,
          narration: "寫成符號就是 2/6：2 在上面是分子、6 在下面是分母。記住口訣——下面數全部、上面數塗色。",
        },
      ],
      quiz: [
        {
        ...
        {
          id: "dn-q3",
          prompt: "一個圓平均分成 5 份、塗色 3 份。這個分數要怎麼寫？",
          options: ["3/5（五分之三）", "5/3（三分之五）", "2/5（五分之二）"],
          answer: 0,
          explain: "分母在下面記「全部 5 份」、分子在上面記「塗色 3 份」，所以寫成 3/5，千萬不要上下顛倒。",
          hints: ["下面那個數要記全部，還是塗色？", "下面＝全部 5 份；上面＝塗色 3 份。", "分母 5 在下、分子 3 在上，是 3/5。"],
          visual: { kind: "wheel", parts: 5, take: 3 },
        },
      ],
    },
    {
      ...
      scenes: [
        ...
        },
        {
          ...
          take: 3,
          ms: 7000,
          narration: "再自己數一次：圓形一共 8 份，8 是分母；塗色 3 份，3 是分子。合起來就是八分之三，寫成 3/8。",
        },
        {
          kind: "bar",
          parts: 4,
          take: 4,
          ms: 6500,
          narration: "如果 4 格全部都塗色，分母 4、分子也是 4，寫成 4/4——整條都拿到了，也就是一整條。",
        },
      ],
      quiz: [
        {
        ...
        {
          id: "ni-q3",
          prompt: "這條巧克力平均分成 8 格、塗色 7 格，只剩 1 格沒塗。塗色部份是幾分之幾？",
          options: ["7/8（八分之七）", "1/8（八分之一）", "8/7（七分之八）"],
          answer: 0,
          explain: "全部 8 格是分母、塗色 7 格是分子，只差 1 格就整條滿了，所以是 7/8。",
          hints: ["先數全部有幾格，放在下面。", "再數塗色幾格，放在上面。", "全部 8、塗色 7，就是 7/8。"],
          visual: { kind: "bar", parts: 8, take: 7 },
        },
      ],
    },
    {
      ...
      scenes: [
        ...
        },
        {
          ...
          take: 2,
          ms: 7500,
          narration: "再看一組：6 格裡塗色 2 格，是 2/6。把 3 格看成一大份，剛好拿到其中 1 份，所以 2/6＝1/3，一樣大。",
        },
        {
          kind: "bar",
          parts: 8,
          take: 4,
          ms: 7500,
          narration: "把 2/4 再切細一倍，變成 8 格、拿 4 格，就是 4/8。分子、分母同時變兩倍，拿到的量不變，所以它們都等值。",
        },
      ],
      quiz: [
        {
        ...
        {
          id: "eq-q3",
          prompt: "一條巧克力切成 8 格、塗色 4 格，塗色的部分和哪一個分數一樣大？",
          options: ["1/2（二分之一）", "1/4（四分之一）", "4/4（四分之四）"],
          answer: 0,
          explain: "8 格塗色 4 格剛好是一半，所以 4/8＝1/2，它們是等值分數。",
          hints: ["8 格塗色 4 格，塗色的是不是剛好一半？", "一半就是二分之一。", "4/8 和 1/2 一樣大。"],
          visual: { kind: "bar", parts: 8, take: 4 },
        },
      ],
    },
  ],
