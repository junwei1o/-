...
import { addGearFragment, baseAttributes, craftGear, createWorldEvent, equipmentBonuses, eventIsActive, GEAR_CATALOG, spendTalentPoint, TALENT_CATALOG, worldEventAllowedAt, worldStateForTime, type TalentId, type WorldEventKind } from "@/game/academyExpansion";
...
export default function RpgAdventure({ onOpenChallenge, questionPool = [], soundEnabled = true }: Props) {
  ...
  const triggerWorldEvent = (kind: WorldEventKind) => {
    ...
    if (!worldEventAllowedAt(kind, now)) return announce("星語觀測需要夜空；等太陽下山再來吧。");
