...
export default function TeacherParentSummary() {
  ...
  return (
    <main className="supporter-summary-page">
      ...
      {emptyProfile ? <section className="supporter-empty-state" aria-live="polite"><BookOpen size={28} aria-hidden="true" /><h2>等待第一段探險足跡</h2><p>學生完成第一題後，五座知識島會依真實紀錄逐步顯示學習方向。現在可以先一起看看地圖。</p><Button type="button" onClick={() => setLocation("/map")}><Compass size={17} /> 前往我的地圖</Button></section> : filteredEmpty ? <section className="supporter-empty-state" aria-live="polite"><CalendarDays size={28} aria-hidden="true" /><h2>這段航線還沒有紀錄</h2><p>可以調整島嶼或日期範圍，回到已有足跡的學習旅程。</p><Button type="button" onClick={clearFilters}><RotateCcw size={17} /> 清除篩選</Button></section> : <><div className="supporter-section-heading"><div><p className="eyebrow">ISLAND LOGBOOK</p><h2>各島嶼學習摘要</h2></div><span><CalendarDays size={15} /> 依本機紀錄更新</span></div><section className="supporter-island-grid" aria-label="五座知識島學習摘要">{summary.islands.map((item) => <IslandCard key={item.island.id} item={item} onOpenMap={(subject) => setLocation(`/map?subject=${encodeURIComponent(subject)}`)} />)}</section></>}
