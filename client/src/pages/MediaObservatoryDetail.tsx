...
export default function MediaObservatoryDetail() {
  ...
  if (!entry) {
    return (
      <main className="region-not-found">
        ...
        <h1>找不到這張觀測卡</h1>
        <Button type="button" onClick={() => setLocation("/observatory")}><ArrowLeft size={16} /> 返回觀測站</Button>
      </main>
    );
  }
  ...
  return (
    <main className={`media-detail-page palette-${entry.palette}`}>
      ...
      <nav className="region-breadcrumb" aria-label="麵包屑導覽"><ol><li><button type="button" onClick={() => setLocation("/")}>航海主頁</button></li><li aria-hidden="true">/</li><li><button type="button" onClick={() => setLocation("/observatory")}>動漫與特攝觀測站</button></li><li aria-hidden="true">/</li><li aria-current="page">{entry.title}</li></ol></nav>
      ...
      {!showQuiz ? <section className="media-detail-quiz-callout" aria-labelledby="anime-quiz-callout-title"><div><p className="eyebrow accent">AFTER OBSERVATION / 探索後挑戰</p><h2 id="anime-quiz-callout-title">用 8 題整理這個世界的線索</h2><p>每題都有觀測焦點與解析，答題後停留在原題，請自己決定何時前進。</p></div><Button type="button" onClick={() => setShowQuiz(true)}><Sparkles size={16} /> 開始 8 題小測驗</Button></section> : <AnimeWorldviewQuiz entryKey={quizKey} title={entry.title} onBack={() => setShowQuiz(false)} onComplete={recordAnimeWorldviewQuizResult} />}
      <a className="media-detail-source" href={entry.officialSourceUrl} target="_blank" rel="noopener noreferrer"><ExternalLink size={14} /> 前往{entry.officialLabel}</a>
