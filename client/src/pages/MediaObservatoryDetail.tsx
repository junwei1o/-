...
export default function MediaObservatoryDetail() {
  ...
  return (
    <main className={`media-detail-page palette-${entry.palette}`}>
      ...
      {!showQuiz ? <section className="media-detail-quiz-callout" aria-labelledby="anime-quiz-callout-title"><div><p className="eyebrow accent">AFTER OBSERVATION / 探索後挑戰</p><h2 id="anime-quiz-callout-title">用 8 題整理這個世界的線索</h2><p>每題都有觀測焦點與解析，答題後停留在原題，請自己決定何時前進。</p></div><button type="button" className="btn primary" onClick={() => setShowQuiz(true)}><Sparkles size={16} /> 開始 8 題小測驗</button></section> : <AnimeWorldviewQuiz entryKey={quizKey} title={entry.title} onBack={() => setShowQuiz(false)} onComplete={recordAnimeWorldviewQuizResult} />}
