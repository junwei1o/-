...
// 弱項標籤環圖色票沿用本頁既有色票（與原寫死 conic-gradient 同組），不新增設計變數。
const ERROR_LABEL_COLORS: Record<string, string> = { 觀念: "#ff6b35", 粗心: "#e6a943", 記憶: "#2e5a4c" };
const ERROR_LABEL_EMPTY = "#e5e8df";
...
export default function LearningReport() {
  ...
  // 環圖由下方圖例同一筆 errorStats 資料即時計算：各分類占比＝自身值／加總，確保圖例數字與環圖一致；總題數為 0 時畫灰環。
  const donutBackground = useMemo(() => {
    const total = errorStats.reduce((sum, item) => sum + item.value, 0);
    if (total <= 0) return `conic-gradient(${ERROR_LABEL_EMPTY} 0 100%)`;
    let cursor = 0;
    const segments = errorStats.map((item) => {
      const start = (cursor / total) * 100;
      cursor += item.value;
      const end = (cursor / total) * 100;
      return `${ERROR_LABEL_COLORS[item.label] ?? "#cbd7ce"} ${start}% ${end}%`;
    });
