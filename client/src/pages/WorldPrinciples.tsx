...
import { ArrowLeft, ArrowUpRight, Atom, Brain, CircleDashed, Compass, Dice5, Gauge, Lightbulb, Orbit, Rainbow, RefreshCw, Scale, Telescope, Waves, Zap, BookOpen, type LucideIcon } from "lucide-react";
import { Link, useLocation } from "wouter";
import PrincipleGuideQuiz from "@/components/PrincipleGuideQuiz";
import { WORLD_PRINCIPLES, type WorldPrincipleKey } from "@/lib/worldPrinciples";
import { ASTRONOMY_EXHIBITS } from "@/lib/astronomy";

const PRINCIPLE_ICONS: Record<WorldPrincipleKey, LucideIcon> = {
  relativity: Orbit,
  "first-principles": Brain,
  quantum: Atom,
  thermodynamics: CircleDashed,
  electromagnetism: Zap,
  "fundamental-forces": Compass,
  "impossible-trinity": Lightbulb,
  leverage: Scale,
  buoyancy: Waves,
  inertia: Gauge,
    ...
    <main className="principles-shell">
      ...
      <section className="wormhole-hero">
        <div className="wormhole-copy">
          ...
          <div className="wormhole-hero-tags"><span>{WORLD_PRINCIPLES.length} 個世界原理</span><span>生活 × 思考 × 科學</span></div>
          <Link href="/astronomy" className="astronomy-entry-link"><Telescope size={17} /><span><strong>進入天文館</strong><small>{ASTRONOMY_EXHIBITS.length} 個展區／觀測任務／宇宙尺度</small></span><ArrowUpRight size={16} /></Link>
        ...
        <div className="principles-grid">
          {WORLD_PRINCIPLES.map((item, index) => {
            ...
            return <button key={item.key} type="button" className={`principle-card principle-${item.color}`} onClick={() => setLocation(`/principles/${item.key}`)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setLocation(`/principles/${item.key}`); } }}>
              <span className="principle-number">{String(index + 1).padStart(2, "0")}</span><span className="principle-icon"><Icon size={20} /></span><p className="eyebrow">{item.eyebrow}</p><h3>{item.name}</h3><span className="principle-english">{item.english}</span><p>{item.short}</p><span className="principle-link">進入觀測 <ArrowUpRight size={15} /></span>
            </button>;
          })}
        </div>
      </section>
