...
const SH_CHEM_ATOM: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 7：游離能大致變大", id: 7, caption: "游離能是「拔走一個電子要多少能量」：同週期由左到右大致變大，所以右邊的氯比左邊的鈉難電離。", action: "jump", prop: { kind: "text", text: "同週期：游離能大致變大", sub: "但 Be→B、N→O 有小波動", tone: "ok" }, duration: 3800, ask: { prompt: "同週期中，哪一邊的游離能通常較大？", options: ["越靠左越大", "越靠右越大", "都一樣大", "不一定"], answer: 1, hint: "核電荷越大，越難拔走電子。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-atom-6", prompt: "同一族（直欄）由上到下，原子半徑與電負度如何變化？", options: ["半徑變小、電負度變大", "半徑變大、電負度變小", "兩者都變大", "兩者都不變"], answer: 1, hints: ["由上到下電子層一層層增加", "核對最外層電子的拉力變弱"], explanation: "同族由上到下電子層數增加，半徑變大、原子抓外層電子的能力變弱，故電負度變小。" },
  ],
};
...
const SH_CHEM_BOND: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 6：鍵越強熔點越高", id: 6, caption: "鍵越強，要破壞它就要越高溫：網狀共價的金剛石熔點約 3550°C，比離子鍵的氯化鈉（801°C）還高。", action: "jump", prop: { kind: "bars", items: [{ label: "氯化鈉", value: 801 }, { label: "金剛石", value: 3550 }], unit: "°C", active: 1 }, duration: 3800, ask: { prompt: "網狀共價的金剛石，熔點通常比離子鍵的氯化鈉如何？", options: ["更低", "更高", "一樣高", "接近 0°C"], answer: 1, hint: "金剛石是全身共價網，極難打破。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-bond-6", prompt: "食鹽水會導電、蔗糖水不導電，主要原因是？", options: ["鹽水比較燙", "氯化鈉溶於水解離出自由離子、蔗糖仍為分子", "蔗糖含有金屬", "兩者其實都導電"], answer: 1, hints: ["導電需要可移動的帶電粒子", "離子化合物溶於水才會解離"], explanation: "NaCl 是離子鍵，溶於水解離成 Na⁺、Cl⁻ 可自由移動而導電；蔗糖是共價分子，溶於水仍為電中性分子，故不導電。" },
  ],
};
...
const SH_CHEM_STOICH: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 4：實驗室怎麼配——三步", id: 4, caption: "把公式變成實驗動作：第一步用天平稱出所需質量，第二步在燒杯加少量水攪拌溶解，第三步倒進容量瓶定量到瓶頸線——量的是「總體積」，不是倒幾毫升水。", action: "walk", prop: { kind: "flow", steps: ["天平稱質量 m", "燒杯中溶解", "容量瓶定量到瓶頸線"], active: 2 }, duration: 4000, ask: { prompt: "在實驗室配製已知濃度的溶液，最後定量時量的是什麼？", options: ["倒進去的水體積", "溶液的總體積", "溶劑質量", "燒杯重量"], answer: 1, hint: "容量瓶上的刻線是「溶液總體積」，不是水的體積。" } },
    ...
    { step: "步驟 6：找出限量試劑", id: 6, caption: "舉例：4 mol H₂ 配 1 mol O₂。按 2:1，1 mol O₂ 只需 2 mol H₂，氫氣用不完，所以氧氣先耗盡，是限量試劑。", action: "jump", prop: { kind: "flow", steps: ["寫出平衡反應式", "算各反應物需要的量", "比對實際擁有的量", "先耗盡的就是限量試劑"], active: 2 }, duration: 3800, ask: { prompt: "4 mol H₂ 與 1 mol O₂ 反應（2H₂＋O₂→2H₂O），誰先耗盡？", options: ["H₂", "O₂", "兩個同時", "都不會"], answer: 1, hint: "1 mol O₂ 只要 2 mol H₂，氫氣綽綽有餘。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-stoichiometry-6", prompt: "理論上可得 10.0 g 產物，實際只得到 8.0 g，產率為？", options: ["80%", "20%", "125%", "8.0%"], answer: 0, hints: ["產率 ＝ 實際 ÷ 理論 ×100%", "8.0 ÷ 10.0 ＝ 0.8"], explanation: "產率 ＝ 實際產量 ÷ 理論產量 ×100% ＝ 8.0 ÷ 10.0 ×100% ＝ 80%。" },
  ],
};
...
const SH_CHEM_ACID_BASE: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 7：中和滴定細拆", id: 7, caption: "實驗室做酸鹼滴定：把已知濃度的鹼慢慢滴進酸裡，邊滴邊搖，加一滴酚酞當眼線。當溶液從無色轉淡紅色、且搖一下不褪色，就是「當量點」。", action: "jump", prop: { kind: "flow", steps: ["酸置於錐形瓶", "加酚酞當指示劑", "由滴定管慢慢滴鹼", "無色轉淡紅＝當量點"], active: 3 }, duration: 4000, ask: { prompt: "酸鹼滴定中，酚酞從無色轉淡紅且搖不褪色，代表什麼？", options: ["酸加太多了", "到達當量點", "指示劑壞了", "可以停止加鹼了"], answer: 1, hint: "此時 H⁺ 與 OH⁻ 幾乎完全中和。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-acid-base-6", prompt: "把濃鹽酸加水稀釋 10 倍，下列何者正確？", options: ["它從強酸變成弱酸", "[H⁺] 變小、pH 上升", "pH 下降 10 個單位", "它從酸變成鹼"], answer: 1, hints: ["稀釋只改變濃度、不改變電離本性", "濃度降為 1/10 ⇒ pH 加 1"], explanation: "強酸稀釋後 [H⁺] 變小、pH 上升（通常升 1 個單位），但它仍是強酸——強弱是電離程度、與濃度無關。" },
  ],
};
...
const SH_CHEM_REDOX: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 6：鐵生鏽就是氧化還原", id: 6, caption: "鐵生鏽 4Fe ＋ 3O₂ → 2Fe₂O₃：鐵從 0 升到 +3 被氧化，氧從 0 降到 −2 被還原，所以它就是氧化還原。", action: "jump", prop: { kind: "balance", left: "4 Fe ＋ 3 O₂", right: "2 Fe₂O₃", tip: "鐵 0 → +3 被氧化" }, duration: 3800, ask: { prompt: "鐵生鏽（4Fe＋3O₂→2Fe₂O₃）時，鐵發生了什麼？", options: ["被還原", "被氧化", "沒變化", "揮發掉"], answer: 1, hint: "鐵從 0 價升到 +3 價。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-redox-6", prompt: "人體呼吸作用（C₆H₁₂O₆＋6O₂→6CO₂＋6H₂O）之所以是氧化還原，是因為？", options: ["它在肺部發生", "葡萄糖中的碳氧化數上升、氧下降，有電子轉移", "它會產生水", "它需要酶"], answer: 1, hints: ["判斷氧化還原看氧化數有無改變", "碳從有機態被氧化成 CO₂"], explanation: "呼吸作用中碳被氧化、氧被還原，氧化數明顯改變，與燃燒、生鏽一樣都是氧化還原，並釋出能量。" },
  ],
};
...
const SH_CHEM_ORGANIC: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 5：生活實例——酒精與醋", id: 5, caption: "廚房就是有機教室：酒精（乙醇）CH₃CH₂OH 掛 –OH 是醇，可氧化成乙醛、再變成醋；醋的主要成分醋酸 CH₃COOH 掛 –COOH，所以帶酸味——官能基一換，性格全變。", action: "walk", prop: { kind: "flow", steps: ["乙醇 –OH（醇）", "氧化成乙醛 –CHO（醛）", "再氧化成醋酸 –COOH（酸）", "官能基一路改變"], active: 2 }, duration: 4200, ask: { prompt: "食醋的酸味來自醋酸 CH₃COOH，它的官能基 –COOH 屬於？", options: ["醇", "醛", "羧酸", "酯"], answer: 2, hint: "–COOH 是羧基，帶酸性。" } },
    { step: "步驟 6：官能基改變性質", id: 6, caption: "官能基直接決定性質：醇能氧化成醛、羧酸帶酸性、酯常有水果香味——認官能基就認得它的脾氣。", action: "jump", prop: { kind: "flow", steps: ["醇 –OH：可氧化、可酯化", "醛 –CHO：易被氧化", "羧酸 –COOH：有酸性", "酯 –COO–：有香味"], active: 0 }, duration: 3800, ask: { prompt: "官能基 –COOH 屬於哪一類有機物？", options: ["醇", "羧酸", "醛", "酮"], answer: 1, hint: "–COOH 是羧基，帶酸性。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-organic-6", prompt: "把紅棕色溴水分別滴入乙烯與乙烷，預期結果是？", options: ["兩者都褪色", "乙烯褪色、乙烷不褪色", "乙烷褪色、乙烯不褪色", "兩者都不褪色"], answer: 1, hints: ["雙鍵可發生化學加成", "烷全是單鍵、不與溴反應"], explanation: "乙烯含 C=C 雙鍵，與溴發生加成反應使溴水褪色；乙烷是飽和烴、只取代不加成，溴水維持紅棕色。" },
  ],
};
...
const SH_CHEM_EQUILIBRIUM: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 4：生活實例——寶特瓶汽水", id: 4, caption: "寶特瓶汽水就是平衡：瓶內 CO₂(g) ⇌ CO₂(aq)。加壓把氣體壓進水裡；一開瓶壓力下降，平衡向左移、CO₂ 從水裡冒出來——就是你看到的氣泡。", action: "walk", prop: { kind: "flow", steps: ["加壓：CO₂ 溶入水中", "密閉達平衡", "開瓶減壓：平衡左移", "CO₂ 冒出成氣泡"], active: 2 }, duration: 4200, ask: { prompt: "打開汽水瓶蓋、壓力下降，溶解的 CO₂ 會怎麼樣？", options: ["溶得更多", "從水中冒出來", "完全不變", "變成固體"], answer: 1, hint: "減壓使平衡往氣體（左）那邊移。" } },
    ...
    { step: "步驟 6：壓力溫度也會推", id: 6, caption: "壓力與溫度也推得動平衡：加壓往氣體分子少的一邊；升溫往吸熱的一邊跑，系統總想抵抗改變。", action: "jump", prop: { kind: "flow", steps: ["加壓→往氣體少的一邊", "升溫→往吸熱的一邊", "減壓→往氣體多的一邊", "降溫→往放熱的一邊"], active: 1 }, duration: 3800, ask: { prompt: "對放熱反應升溫，平衡會往哪邊移？", options: ["放熱方向", "吸熱方向", "不動", "停止"], answer: 1, hint: "升溫系統想吸熱來降溫。" } },
  ...
  questions: [
    ...
    { id: "sh-chem-equilibrium-6", prompt: "打開汽水瓶蓋後氣泡冒出，最符合哪一項原理？", options: ["催化劑加速反應", "減壓使 CO₂(g)⇌CO₂(aq) 平衡向左移", "溫度突然下降", "氣泡與平衡無關"], answer: 1, hints: ["開瓶壓力下降", "系統想生成更多氣體"], explanation: "密閉加壓時 CO₂ 多溶於水；開瓶減壓，平衡向生成氣體（左）的方向移動，CO₂ 逸出形成氣泡。" },
  ],
};
...
const SH_CHEM_SOLUTION: OnionLesson = {
  ...
  questions: [
    ...
    { id: "sh-chem-solution-6", prompt: "為什麼配製 1.00 M、1 L 的食鹽水時，要在容量瓶中加水到刻線而不是直接加 1 L 水？", options: ["因為鹽會吸水", "因為 1 L 水＋鹽的總體積未必正好 1 L，刻線才是溶液總體積", "因為鹽不溶於水", "因為越稀越好"], answer: 1, hints: ["溶質本身也佔體積", "刻線定義的是溶液總體積"], explanation: "加入溶質後總體積會改變，且液體混合體積不一定可加；容量瓶刻線定義的是溶液總體積恰為 1 L，才能得到準確的 1.00 M。" },
  ],
};
