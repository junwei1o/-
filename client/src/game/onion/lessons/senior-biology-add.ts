...
const SH_BIO_CELL: OnionLesson = {
  ...
  frames: [
    ...
    { id: 6, step: "步驟 6：主動運輸要花 ATP", caption: "主動運輸逆著濃度梯度送物質，像扛東西上樓，必須消耗 ATP 能量才辦得到。", action: "jump", prop: { kind: "text", text: "主動運輸：逆梯度、要 ATP", sub: "運輸蛋白協助、會耗能", tone: "ok" }, duration: 3600, ask: { prompt: "下列哪一種運輸方式需要消耗 ATP？", options: ["擴散", "滲透", "主動運輸", "都不需要"], answer: 2, hint: "主動運輸逆濃度梯度，要耗能；擴散與滲透是免費的被動運輸。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-cell-6", prompt: "把紅血球放入濃食鹽水中，它通常會？", options: ["吸水脹大", "維持原狀", "失水萎縮", "分裂成兩個"], answer: 2, hints: ["水由低濃度往高濃度跑", "細胞內水分往外滲透"], explanation: "濃食鹽水張力高，水從紅血球內部滲透出去，紅血球失水萎縮（在清水才會脹破）。" },
  ],
};
...
const SH_BIO_GENETICS: OnionLesson = {
  ...
  frames: [
    ...
    { id: 6, step: "步驟 6：棋盤方格驗算", caption: "用棋盤方格把 T 和 t 排成橫直兩邊，四格分別是 TT、Tt、Tt、tt，一眼看清比例。", action: "jump", prop: { kind: "flow", steps: ["橫列 T、t", "直行 T、t", "交叉得四格", "TT、Tt、Tt、tt"], active: 3 }, duration: 3600, ask: { prompt: "棋盤方格中，Tt 這個基因型出現了幾格？", options: ["1 格", "2 格", "3 格", "4 格"], answer: 1, hint: "四格為 TT、Tt、Tt、tt，Tt 佔其中兩格。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-genetics-6", prompt: "把一株高莖豌豆與矮莖（tt）雜交，後代約有一半高莖、一半矮莖，這株高莖的基因型是？", options: ["TT", "Tt", "tt", "無法判斷"], answer: 1, hints: ["出現矮莖代表它有帶 t", "TT 與 tt 雜交後代應全高莖"], explanation: "後代出現矮莖 tt，代表親本高莖必須帶有 t，故為 Tt（Tt × tt → Tt:tt ＝ 1:1）。" },
  ],
};
...
const SH_BIO_DNA: OnionLesson = {
  ...
  frames: [
    ...
    { id: 6, step: "步驟 6：轉譯與密碼子", caption: "轉譯在核糖體進行：每 3 個鹼基組成一個密碼子，對應 1 個胺基酸，像三鍵一字的密碼。", action: "jump", prop: { kind: "text", text: "密碼子：3 個鹼基 = 1 個胺基酸", sub: "AUG 是起始密碼子", tone: "ok" }, duration: 3600, ask: { prompt: "mRNA 上幾個相鄰鹼基組成一個密碼子？", options: ["1 個", "2 個", "3 個", "4 個"], answer: 2, hint: "三個鹼基（一個密碼子）對應一個胺基酸。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-dna-6", prompt: "DNA 鑑定（如刑事、親子鑑定）能區分兩個人，主要依據是？", options: ["所有人 DNA 長度不同", "每個人（除同卵雙胞胎）的 DNA 鹼基序列不同", "DNA 在細胞核裡", "DNA 會變色"], answer: 1, hints: ["序列就像個人的條碼", "同卵雙胞胎序列才會相同"], explanation: "每個人（除同卵雙胞胎）DNA 鹼基序列都獨特，切出片段比對條紋即可指認身分。" },
  ],
};
...
const SH_BIO_EVOLUTION: OnionLesson = {
  ...
  frames: [
    ...
    { id: 5, step: "步驟 5：抗藥性例子", caption: "例子：細菌本有抗藥個體，用抗生素後敏感菌死、抗藥菌留下，抗藥性就擴散開來。", action: "walk", prop: { kind: "flow", steps: ["族群本有抗藥變異", "抗生素殺敏感菌", "抗藥菌存活", "抗藥性擴散"], active: 3 }, duration: 3600, ask: { prompt: "濫用抗生素為什麼會讓抗藥菌變多？", options: ["抗生素訓練細菌變強", "它殺死敏感菌、留下抗藥菌", "細菌主動變抗藥", "與抗生素無關"], answer: 1, hint: "藥物篩選出原本就存在的抗藥個體，讓它們留下繁殖。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-evolution-6", prompt: "「長頸鹿為了吃高處葉子而伸長脖子，後代脖子就變長」這種觀點屬於？", options: ["正確的天擇說", "拉馬克的用進廢退（後天獲得性狀遺傳）", "基因突變", "現代綜合演化論"], answer: 1, hints: ["把『使用』和『遺傳』混在一起", "後天努力不會改變基因"], explanation: "這是拉馬克主張的用進廢退／後天獲得性狀遺傳，現代演化論已不採信；長頸鹿長頸是天擇保留長頸變異的結果。" },
  ],
};
...
const SH_BIO_ECOLOGY: OnionLesson = {
  ...
  questions: [
    ...
    { id: "sh-bio-ecology-6", prompt: "DDT 等難分解毒物，為何頂級掠食者體內濃度最高？", options: ["牠們比較愛吃髒東西", "毒物隨食物鏈逐層累積（生物放大）", "牠們代謝很快", "DDT 會自己蒸發上去"], answer: 1, hints: ["毒物吃進去排不出來", "每一層再往上累積一層"], explanation: "難分解毒物被生物吸收後不易排出，隨營養階層逐層累積放大，頂級掠食者吃下多層生物，體內濃度最高。" },
  ],
};
...
const SH_BIO_PLANT_PHYSIOLOGY: OnionLesson = {
  ...
  frames: [
    ...
    { id: 5, step: "步驟 5：蒸散產生拉力", caption: "葉片氣孔散失水氣產生拉力，把根部的水經木質部一路拉到葉片，這叫蒸散拉力。", action: "walk", prop: { kind: "flow", steps: ["根吸水", "經木質部上行", "葉氣孔散失水", "產生向上拉力"], active: 3 }, duration: 3600, ask: { prompt: "植物把水分從根運到葉的主要動力來自？", options: ["根部主動加壓", "葉片蒸散產生的拉力", "重力推動", "光合作用直接抽水"], answer: 1, hint: "蒸散使葉散失水氣，形成向上的拉力（蒸散拉力）。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-plant-physiology-6", prompt: "葉片澱粉實驗中，照光半邊滴碘液變成藍黑色，是因為？", options: ["碘液本身是藍黑色", "照光行光合作用製造了澱粉，澱粉遇碘變藍黑色", "酒精把葉子染藍", "葉綠素是藍黑色"], answer: 1, hints: ["澱粉遇碘變色是檢驗原理", "暗處理後只剩新造的澱粉"], explanation: "光合作用產生的澱粉遇碘液會變藍黑色；照光半邊能行光合作用，故變色，遮光半邊不能。" },
  ],
};
...
const SH_BIO_HUMAN_BODY: OnionLesson = {
  ...
  frames: [
    ...
    { id: 5, step: "步驟 5：負回饋維恆定", caption: "以體溫為例：變熱就出汗散熱、降回 37°C；偏離就被拉回，這叫負回饋。", action: "walk", prop: { kind: "cycle", nodes: ["偏離設定點", "偵測", "啟動相反反應", "回到恆定", "再監測"], active: 0 }, duration: 3600, ask: { prompt: "人體維持體溫、血糖等穩定，主要靠下列哪一種機制？", options: ["正回饋", "負回饋", "完全不調節", "隨機波動"], answer: 1, hint: "負回饋：偏離時啟動相反作用把數值拉回設定點。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-human-body-6", prompt: "分娩時子宮收縮越來越強、直到胎兒生出，屬於哪種機制？", options: ["負回饋", "正回饋", "恆定作用", "隨機波動"], answer: 1, hints: ["結果強化原本的反應", "一路加速直到完成"], explanation: "分娩時收縮刺激催產素、催產素又加強收縮，是「結果強化原反應」的正回饋；與拉回設定點的負回饋方向相反。" },
  ],
};
...
const SH_BIO_BIOTECH: OnionLesson = {
  ...
  frames: [
    ...
    { id: 5, step: "步驟 5：PCR 溫度曲線細拆", caption: "一個循環要換三次溫度：95°C 高熱把雙股拆開、降到約 55°C 讓引子貼上模板、再升到約 72°C 讓聚合酶把新股補齊——酶要耐熱，否則第一輪就被煮熟了。", action: "jump", prop: { kind: "flow", steps: ["95°C：解鏈", "55°C：引子結合", "72°C：延伸合成", "需耐熱 DNA 聚合酶"], active: 3 }, duration: 4200, ask: { prompt: "PCR 循環中，讓引子與模板互補配對（退火）發生在約多少度？", options: ["95°C", "55°C", "72°C", "0°C"], answer: 1, hint: "高溫解鏈、降溫才配對、中溫延伸。" } },
  ...
  questions: [
    ...
    { id: "sh-bio-biotech-6", prompt: "PCR 反覆加熱到 95°C 還能運作，是因為反應中使用了？", options: ["常溫酵素即可", "耐熱的 DNA 聚合酶", "大量冰塊降溫", "不需要酶"], answer: 1, hints: ["一般酶高熱就變性", "耐熱酶來自嗜熱菌"], explanation: "PCR 每輪都要高溫解鏈，一般 DNA 聚合酶會被熱破壞，故需耐熱聚合酶（如 Taq 酶）才能反覆循環。" },
  ],
};
