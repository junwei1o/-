...
const SH_ENG_TENSES: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 5：進行式例句", id: 5, caption: "進行式強調動作正在發生。正在吃：I am eating.；那時在吃：I was eating.；將在吃：I will be eating.", ask: { prompt: "「Look! He ___ dinner now.」空格要用？", options: ["is cooking", "cooks", "cooked", "will cook"], answer: 0, hint: "now 與 Look! 提示正在發生，用 be 動詞 + V-ing。" }, action: "point", prop: { kind: "text", text: "進行式：正在發生", sub: "I am eating. / I was eating. / I will be eating.", tone: "ok" }, duration: 3800 },
  ...
  questions: [
    { id: "sh-eng-tenses-1", prompt: "英文一共有幾個時態？", options: ["4", "12", "8", "16"], answer: 1, hints: ["時間有 3 條線", "每條線 4 種看法，3 × 4 = ?"], explanation: "時間（過去/現在/未來）三線，乘上四種看法（簡單/進行/完成/完成進行），共 12 個時態。" },
    { id: "sh-eng-tenses-2", prompt: "「I eat breakfast every day.」屬於哪個時態？", options: ["現在進行式", "過去式", "現在簡單式", "未來式"], answer: 2, hints: ["every day 是習慣", "簡單式講習慣或事實"], explanation: "every day 表示習慣，動詞用原形 eat，是現在簡單式。" },
    { id: "sh-eng-tenses-3", prompt: "「Look! It is raining.」用了什麼結構？", options: ["have + p.p.", "will + 原形", "動詞原形", "be 動詞 + V-ing"], answer: 3, hints: ["Look! 表示正在發生", "進行式 = be + V-ing"], explanation: "is raining 是 be 動詞 + V-ing，配合 Look! 表示此刻正在下雨，是現在進行式。" },
    { id: "sh-eng-tenses-4", prompt: "「I have finished my homework.」強調什麼？", options: ["到目前為止已經完成", "過去某時點發生", "未來才要做", "動作正在進行"], answer: 0, hints: ["have + p.p. 是完成式", "強調對現在的影響或結果"], explanation: "have finished 是現在完成式，強調到目前為止已經完成的經驗或結果。" },
    ...
    { id: "sh-eng-tenses-6", prompt: "「She ___ in Taipei since 2020.」空格用？", options: ["lived", "lives", "has lived", "is living"], answer: 2, hints: ["since 2020 表持續到現在", "用 have/has + p.p."], explanation: "since 2020 表示從過去持續到現在，要用現在完成式 has lived。" },
    { id: "sh-eng-tenses-7", prompt: "「every morning」這個時間副詞片語，通常提示哪個時態？", options: ["現在進行式", "未來式", "過去式", "現在簡單式"], answer: 3, hints: ["every morning 是規律習慣", "習慣用現在簡單式"], explanation: "every morning 表示每天固定的習慣，屬於現在簡單式的時間線索。" },
  ],
};
...
const SH_ENG_SUBJUNCTIVE: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 3：記住 be 用 were", id: 3, caption: "注意 be 動詞要用 were（不分人稱）：If I were you, I would take the job. 表示與現在事實相反。", ask: { prompt: "與現在事實相反時，if 子句的 be 動詞要用？", options: ["was / were（都用 were）", "am / is / are", "will be", "be 原形"], answer: 0, hint: "假設語氣中 be 不分人稱，一律用 were。" }, action: "think", prop: { kind: "text", text: "If I were you, I would take the job.", sub: "與現在相反：if + 過去式, would + 原形", tone: "ok" }, duration: 3800 },
  ...
  questions: [
    ...
    { id: "sh-eng-subjunctive-3", prompt: "「If I had known it, I ___ .」空格要用？", options: ["will tell", "would tell", "told", "would have told"], answer: 3, hints: ["had known 是過去完成式", "主句用 would have + p.p."], explanation: "if 子句 had known 是過去完成式，主句要用 would have + p.p.，即 would have told。" },
    { id: "sh-eng-subjunctive-4", prompt: "「If it rains tomorrow, we ___ stay home.」空格要用？", options: ["will", "would", "would have", "had"], answer: 0, hints: ["未來真有可能發生", "主句用 will"], explanation: "這是未來有可能發生的真實條件句，if 子句用現在式 rains，主句用 will stay home。" },
    ...
    { id: "sh-eng-subjunctive-6", prompt: "「___ you, I would tell her the truth.」省略 if 時開頭用？", options: ["Do", "Are", "Were", "Would"], answer: 2, hints: ["省略 if 要倒裝", "Were I you = If I were you"], explanation: "省略 if 時動詞往前移，Were you（＝If I were you）是與現在事實相反的倒裝寫法。" },
    { id: "sh-eng-subjunctive-7", prompt: "「If she ___ harder last year, she would have passed.」空格用？", options: ["studies", "studied", "study", "had studied"], answer: 3, hints: ["last year 是過去", "過去相反用 had + p.p."], explanation: "與過去事實相反，if 子句用過去完成式 had studied，主句用 would have passed。" },
  ],
};
...
const SH_ENG_PARTICIPLE: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 5：分詞構句由來", id: 5, caption: "分詞構句是把副詞子句簡化：When I was walking home, I saw a dog. 可寫成 Walking home, I saw a dog.", ask: { prompt: "「___ home, I saw a dog.」開頭要用？", options: ["Walking", "Walk", "Walked", "To walk"], answer: 0, hint: "主詞 I 是「走」的主動者，用現在分詞 Walking。" }, action: "point", prop: { kind: "flow", steps: ["副詞子句 When I was walking home", "簡化成 Walking home", "主詞必須一致"], active: 1 }, duration: 4000 },
    ...
    { step: "步驟 7：主動用 V-ing、被動用 p.p.", id: 7, caption: "分詞構句選哪種，看主詞和動作的關係：主詞「做」這個動作就用 V-ing；主詞「被」這個動作就用 p.p.。", ask: { prompt: "「___ from the hill, the city looks small.」（city 被看）用？", options: ["Seeing", "Seen", "See", "To see"], answer: 1, hint: "city 是「被看」，用過去分詞 Seen。" }, action: "think", prop: { kind: "flow", steps: ["主動：主詞做 → V-ing", "被動：主詞被 → p.p.", "Walking... / Seen..."], active: 1 }, duration: 3600 },
    { step: "步驟 8：當心懸垂分詞", id: 8, caption: "小心懸垂分詞！若寫 Walking home, a dog bit me.，會變成狗在走回家，這就叫主詞不一致。", ask: { prompt: "為什麼「Walking home, a dog bit me.」是錯的？", options: ["主詞 a dog 不是走的人，形成懸垂分詞", "dog 不該用 a", "walking 拼錯", "bit 時態錯"], answer: 0, hint: "分詞的主詞必須與主句主詞一致。" }, action: "think", prop: { kind: "text", text: "懸垂分詞：a dog 沒在走", sub: "分詞的主詞要和主句主詞一致", tone: "warn" }, duration: 3800 },
  ...
  questions: [
    ...
    { id: "sh-eng-participle-3", prompt: "「When I heard the news, I cried.」可簡化為？", options: ["Heard the news, I cried.", "Hear the news, I cried.", "To hear the news, I cried.", "Hearing the news, I cried."], answer: 3, hints: ["I 是 hear 的主動者", "用現在分詞 Hearing"], explanation: "I 是 hear 的主動者，副詞子句可簡化為 Hearing the news, I cried." },
    ...
    { id: "sh-eng-participle-5", prompt: "下列哪一句有懸垂分詞的錯誤？", options: ["Walking in the park, I saw a flower.", "Reading the book, a phone rang.", "Born in Taipei, she speaks Taiwanese.", "Tired, he went to bed."], answer: 1, hints: ["a phone 不是讀書的人", "主詞不一致"], explanation: "Reading the book, a phone rang. 中 phone 不會讀書，形成懸垂分詞；應改為 Reading the book, I heard a phone ring." },
    { id: "sh-eng-participle-6", prompt: "「The ___ cup on the floor is hers.」（杯子被打破）用？", options: ["breaking", "break", "broken", "broke"], answer: 2, hints: ["杯子被打破", "被動用 p.p."], explanation: "broken 是 break 的過去分詞，表被動、完成，意思是「被打破的杯子」。" },
    { id: "sh-eng-participle-7", prompt: "「___ the door, he walked in.」（他主動開門）用？", options: ["Opened", "Open", "Opens", "Opening"], answer: 3, hints: ["he 是「開門」的主動者", "主動用 V-ing"], explanation: "he 是開門這個動作的主動者，副詞子句可簡化為 Opening the door, he walked in." },
  ],
};
...
const SH_ENG_READING_WRITING: OnionLesson = {
  ...
  frames: [
    ...
    { step: "步驟 5：段落寫作結構", id: 5, caption: "段落寫作有固定結構：先用主題句點題，再給 supporting details，最後用結論句收尾。", ask: { prompt: "一個標準段落的順序是？", options: ["主題句→細節→結論句", "細節→主題→結論", "結論→細節→主題", "細節→結論→主題"], answer: 0, hint: "先點題，再支持，最後收尾。" }, action: "point", prop: { kind: "flow", steps: ["topic sentence 主題句", "supporting details 細節", "concluding sentence 結論"], active: 0 }, duration: 4000 },
  ...
  questions: [
    ...
    { id: "sh-eng-reading-writing-2", prompt: "「however」在文中通常表示？", options: ["因果", "舉例", "轉折", "總結"], answer: 2, hints: ["however = 但是", "表示意思轉彎"], explanation: "however 是轉折詞，表示前後意思相反或轉彎，等同中文的「然而、但是」。" },
    { id: "sh-eng-reading-writing-3", prompt: "寫一個段落時，正確的結構是？", options: ["細節→結論→主題句", "結論→主題→細節", "細節→主題→結論", "主題句→細節→結論句"], answer: 3, hints: ["先點題", "再支持、最後收尾"], explanation: "標準段落為：主題句開頭點題，supporting details 提供支持，concluding sentence 收尾。" },
    { id: "sh-eng-reading-writing-4", prompt: "寫摘要時應該？", options: ["留主張與結論、刪例子", "保留所有例子", "照抄第一段", "越長越好"], answer: 0, hints: ["摘要要濃縮", "保留主張和結論"], explanation: "摘要只保留作者主張與結論，刪去例子和細節，濃縮成簡短幾句。" },
    { id: "sh-eng-reading-writing-5", prompt: "下列哪個做法符合正確的轉述？", options: ["直接複製原句", "改寫句型與詞彙、意思不變", "只改幾個字但結構相同", "翻譯成中文再翻回英文"], answer: 1, hints: ["轉述要改寫", "意思必須相同"], explanation: "正確轉述要改寫句型並替換詞彙，但意思保持不變，以避免抄襲之嫌。" },
    { id: "sh-eng-reading-writing-6", prompt: "若主題句不在段首，要怎麼找段旨？", options: ["數句子長短", "只看標點符號", "看全段反覆強調的概念", "看最後一個字"], answer: 2, hints: ["反覆出現的就是重點", "把它濃縮成一句"], explanation: "主題句常點出全段核心概念，找不著時就看全段反覆強調的那句，即為段旨。" },
    { id: "sh-eng-reading-writing-7", prompt: "判斷文章主旨時，哪種選項要先刪掉？", options: ["能涵蓋各段論點的選項", "與標題呼應的選項", "作者的核心主張", "只涵蓋單一例子的過窄選項"], answer: 3, hints: ["太窄＝只抓到例子", "主旨要涵蓋全文"], explanation: "判斷主旨時，只涵蓋單一例子的選項太窄、超出全文範圍的選項太寬，都應刪除。" },
  ],
};
