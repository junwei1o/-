...
export const UNIT_CONVERSION_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！公分、公尺、公里常常搞混嗎？今天把它們排在一起比比看。", action: "wave", prop: { kind: "none" }, duration: 2800 },
    { id: 2, caption: "先看最小的公分：手指寬度大約 1 公分，量鉛筆、橡皮擦都用它。", action: "point", prop: { kind: "bars", items: [{ label: "1公分", value: 1 }, { label: "10公分", value: 10 }], unit: "公分" }, duration: 3200 },
    { id: 3, caption: "100 個 1 公分接起來，就是 1 公尺——大概是一張書桌的高度。", ask: { prompt: "1 公尺等於幾公分？", options: ["10 公分", "100 公分", "1000 公分", "10000 公分"], answer: 1, hint: "「公尺」的「厘」就是百分之一，1 公尺 = 100 公分。" }, action: "jump", prop: { kind: "bars", items: [{ label: "1公分", value: 1 }, { label: "1公尺", value: 100 }], unit: "公分", active: 1 }, duration: 3600 },
    { id: 4, caption: "再上去是公里：1000 個 1 公尺接起來才是 1 公里，走路大概要 15 分鐘。", ask: { prompt: "1 公里等於幾公尺？", options: ["100 公尺", "10 公尺", "1000 公尺", "10000 公尺"], answer: 2, hint: "「公里」的「千」就是一千，1 公里 = 1000 公尺。" }, action: "walk", prop: { kind: "bars", items: [{ label: "1公尺", value: 1 }, { label: "1公里", value: 1000 }], unit: "公尺", active: 1 }, duration: 3600 },
    { id: 5, caption: "記住這條樓梯：公里 →（×1000）→ 公尺 →（×100）→ 公分。往下走就乘以進率。", action: "point", prop: { kind: "flow", steps: ["公里", "×1000", "公尺", "×100", "公分"], active: 2 }, duration: 3600 },
    { id: 6, caption: "反過來往上走，就要「除以」進率：公分 ÷100 變公尺、公尺 ÷1000 變公里。", action: "think", prop: { kind: "flow", steps: ["公分", "÷100", "公尺", "÷1000", "公里"], active: 1 }, duration: 3600 },
    { id: 7, caption: "口訣：大換小、用乘法；小換大、用除法。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const FACTOR_MULTIPLE_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！有 12 顆糖，要平分給幾個人才能剛好分完、沒有剩下？", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "分給 1 個人：12 顆全給他。12 ÷ 1 = 12，可以整除。", action: "point", prop: { kind: "bars", items: [{ label: "1人", value: 12 }, { label: "2人", value: 6 }], unit: "顆" }, duration: 3200 },
    { id: 3, caption: "分給 2 個人：每人 6 顆。分給 3 個人：每人 4 顆。都剛好分完！", ask: { prompt: "12 ÷ 5 能不能整除？", options: ["可以，每人 2 顆", "可以，每人 3 顆", "不行，會剩下 2 顆", "不行，會剩下 1 顆"], answer: 2, hint: "5 × 2 = 10，還剩下 2 顆分不掉。" }, action: "think", prop: { kind: "bars", items: [{ label: "3人", value: 4 }, { label: "5人", value: 2 }], unit: "顆", active: 1 }, duration: 3600 },
    { id: 4, caption: "像 1、2、3、4、6、12 這些「能整除 12」的數，就是 12 的因數。", action: "point", prop: { kind: "text", text: "12 的因數：1, 2, 3, 4, 6, 12", sub: "能整除 12 的數，一共 6 個", tone: "ok" }, duration: 3600 },
    { id: 5, caption: "反過來看：12、24、36、48 一直加 12 上去，這些就是 12 的倍數。", ask: { prompt: "下面哪一個不是 12 的倍數？", options: ["24", "36", "60", "30"], answer: 3, hint: "12 × 2 = 24、12 × 3 = 36、12 × 5 = 60。", }, action: "walk", prop: { kind: "bars", items: [{ label: "12", value: 12 }, { label: "24", value: 24 }, { label: "36", value: 36 }], unit: "" }, duration: 3600 },
    { id: 6, caption: "關鍵差異：因數數得完（有限個），倍數一直乘下去數不完（無限多個）。", action: "jump", prop: { kind: "text", text: "因數有限 ‧ 倍數無限", sub: "12 的因數 6 個；12 的倍數數不完", tone: "ok" }, duration: 3600 },
    { id: 7, caption: "口訣：因數能整除、倍數一直加。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const FRACTION_MULTIPLY_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！一塊蛋糕切成 4 等份，你拿了其中的 1 份，就是 1/4 塊。", action: "wave", prop: { kind: "pie", a: 1, b: 4, label: "1/4" }, duration: 3200 },
    { id: 2, caption: "如果有 3 個這樣的 1/4，就把它們疊起來看看。", action: "point", prop: { kind: "pies", left: { a: 1, b: 4 }, right: { a: 1, b: 4 }, result: { a: 2, b: 4 } }, duration: 3400 },
    { id: 3, caption: "1/4 ＋ 1/4 ＋ 1/4 ＝ 3/4。所以 1/4 × 3 ＝ 3/4！", ask: { prompt: "1/4 × 3 等於多少？", options: ["3/4", "3/12", "1/12", "4/3"], answer: 0, hint: "分母都是 4，只把分子 1×3 = 3。" }, action: "jump", prop: { kind: "pie", a: 3, b: 4, label: "3/4" }, duration: 3600 },
    { id: 4, caption: "規則很簡單：分數乘以整數，只把「分子」乘以整數，分母不動。", action: "point", prop: { kind: "text", text: "a/b × n = (a×n)/b", sub: "分子乘以整數，分母保持不變", tone: "ok" }, duration: 3600 },
    { id: 5, caption: "試試看 2/5 × 3：分子 2×3 = 6，分母還是 5，答案是 6/5。", ask: { prompt: "2/5 × 3 等於多少？", options: ["6/5", "6/15", "5/6", "2/15"], answer: 0, hint: "分子 2×3 = 6，分母 5 不變。" }, action: "think", prop: { kind: "pie", a: 6, b: 5, label: "6/5" }, duration: 3600 },
    { id: 6, caption: "6/5 是假分數（分子比分母大），換成帶分數是 1 又 1/5。", action: "cheer", prop: { kind: "text", text: "6/5 = 1 又 1/5", sub: "分子 ÷ 分母：6 ÷ 5 = 1 餘 1", tone: "ok" }, duration: 3600 },
    { id: 7, caption: "口訣：乘整數、動分子；分母不動。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const PUNCTUATION_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！寫句子的時候，標點符號就像馬路上的紅綠燈，告訴讀者哪裡要停。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "逗號「，」是黃燈：話還沒說完，先喘一口氣再繼續。", ask: { prompt: "「今天天氣很好__我們去公園玩。」空格要放什麼？", options: ["句號。", "逗號，", "問號？", "驚嘆號！"], answer: 1, hint: "話還沒說完，後面還有「我們去公園玩」。" }, action: "point", prop: { kind: "text", text: "今天天氣很好，我們去公園玩。", sub: "逗號＝黃燈，話還沒說完", tone: "ok" }, duration: 3600 },
    { id: 3, caption: "句號「。」是紅燈：這句話說完了，到此為止。", action: "walk", prop: { kind: "text", text: "我們去公園玩。", sub: "句號＝紅燈，句子結束", tone: "ok" }, duration: 3400 },
    { id: 4, caption: "問號「？」用在發問：句子裡有「誰、什麼、哪裡、嗎、呢」通常就是問句。", ask: { prompt: "「你今天要不要去圖書館__」空格要放什麼？", options: ["句號。", "逗號，", "問號？", "驚嘆號！"], answer: 2, hint: "這句話是在問別人的意見。" }, action: "think", prop: { kind: "text", text: "你今天要不要去圖書館？", sub: "問號＝有問題要問", tone: "ok" }, duration: 3600 },
    { id: 5, caption: "驚嘆號「！」用在有強烈情緒：驚訝、生氣、開心大叫都用它。", action: "jump", prop: { kind: "text", text: "這朵花好漂亮啊！", sub: "驚嘆號＝有情緒、很大聲", tone: "ok" }, duration: 3400 },
    { id: 6, caption: "四種燈號記起來了嗎？黃燈逗號、紅燈句號、問號發問、驚嘆有情緒。", action: "point", prop: { kind: "flow", steps: ["，還沒說完", "。說完了", "？要發問", "！有情緒"], active: 3 }, duration: 3600 },
    { id: 7, caption: "口訣：停一下用逗號，說完用句號，發問用問號，大叫用驚嘆號。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const FOOD_CHAIN_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！草原上有草、有兔子、有老鷹，牠們之間藏著一條看不見的線。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "兔子吃草，所以能量從「草」跑到「兔子」身上。箭頭要畫成 草 → 兔子。", ask: { prompt: "食物鏈裡的箭頭代表什麼？", options: ["誰被誰吃", "能量的流向（被誰吃掉）", "誰比較大隻", "誰跑得比較快"], answer: 1, hint: "箭頭指向「把對方吃掉」的那一方。" }, action: "point", prop: { kind: "flow", steps: ["草", "兔子"], active: 1 }, duration: 3600 },
    { id: 3, caption: "老鷹又吃掉兔子，能量繼續往上傳：草 → 兔子 → 老鷹。", action: "walk", prop: { kind: "flow", steps: ["草", "兔子", "老鷹"], active: 2 }, duration: 3600 },
    { id: 4, caption: "第一棒幾乎都是綠色植物，叫做「生產者」——它們自己用陽光製造養分。", action: "point", prop: { kind: "cycle", nodes: ["生產者", "消費者"], active: 0 }, duration: 3400 },
    { id: 5, caption: "吃別人的叫做「消費者」：吃植物的是草食性，吃動物的是肉食性。", ask: { prompt: "兔子在食物鏈裡是什麼角色？", options: ["生產者", "草食性消費者", "肉食性消費者", "分解者"], answer: 1, hint: "兔子吃草，自己不會製造養分。" }, action: "think", prop: { kind: "cycle", nodes: ["生產者", "消費者"], active: 1 }, duration: 3600 },
    { id: 6, caption: "還有一群默默工作的「分解者」：黴菌、細菌把屍體和落葉分解回土壤。", action: "jump", prop: { kind: "text", text: "生產者 → 消費者 → 分解者", sub: "養分最後回到土壤，給植物再用", tone: "ok" }, duration: 3600 },
    { id: 7, caption: "口訣：箭頭指向吃的一方，植物開頭、動物接棒、細菌收尾。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const STAT_CHART_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！老師調查全班最喜歡的水果，結果要怎麼一眼看出來？", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "把數量畫成長條圖：每種水果一根長條，越高代表喜歡的人越多。", action: "point", prop: { kind: "bars", items: [{ label: "蘋果", value: 8 }, { label: "香蕉", value: 5 }, { label: "葡萄", value: 12 }], unit: "人" }, duration: 3600 },
    { id: 3, caption: "第一步：先看「標題」和「單位」，才知道這張圖在比什麼、一格是多少。", ask: { prompt: "看長條圖時，第一步要先看什麼？", options: ["最長的那根", "標題和單位", "顏色", "圖的大小"], answer: 1, hint: "不知道單位，就不知道一格代表多少。" }, action: "think", prop: { kind: "text", text: "標題：最喜歡的水果　單位：1 格 = 1 人", sub: "先讀標題與單位", tone: "ok" }, duration: 3600 },
    { id: 4, caption: "第二步：找最長和最矮的長條，就知道「最多」和「最少」。", action: "point", prop: { kind: "bars", items: [{ label: "葡萄", value: 12 }, { label: "香蕉", value: 5 }], unit: "人", active: 0 }, duration: 3400 },
    { id: 5, caption: "第三步：要比「相差多少」，就用長的減掉短的：12 − 5 = 7 人。", ask: { prompt: "喜歡葡萄（12 人）比香蕉（5 人）多幾人？", options: ["7 人", "17 人", "5 人", "12 人"], answer: 0, hint: "相差要用減法：12 − 5。" }, action: "jump", prop: { kind: "text", text: "12 − 5 = 7（人）", sub: "比多少用減法", tone: "ok" }, duration: 3600 },
    { id: 6, caption: "要算「總共多少人」，就把每一根長條的數量全部加起來。", action: "walk", prop: { kind: "bars", items: [{ label: "蘋果", value: 8 }, { label: "香蕉", value: 5 }, { label: "葡萄", value: 12 }], unit: "人" }, duration: 3400 },
    { id: 7, caption: "口訣：先讀標題單位，再看最長最短，相差用減、總共用加。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const PHOTOSYNTHESIS_JUNIOR_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！國小我們把葉子當成一整座工廠，今天要拆開葉綠體，看裡面有幾條生產線。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "葉綠體內部有兩區：堆疊起來的「葉綠餅」和周圍的液體「基質」，各管一條生產線。", action: "point", prop: { kind: "flow", steps: ["葉綠餅", "基質"], active: 0 }, duration: 3400 },
    { id: 3, caption: "第一條：光反應——一定要有光。水被拆開，釋放出氧氣，同時把光能存成能量貨幣 ATP。", ask: { prompt: "光合作用產生的氧氣，來自哪個物質被分解？", options: ["二氧化碳", "水", "葡萄糖", "葉綠素"], answer: 1, hint: "光反應把「水」拆開，氧氣是水的副產品。" }, action: "think", prop: { kind: "flow", steps: ["光能", "水被拆解", "釋出 O₂", "存成 ATP"], active: 2 }, duration: 3800 },
    { id: 4, caption: "第二條：暗反應——在基質進行，有沒有光都能跑（但需要光反應給的 ATP）。", action: "walk", prop: { kind: "flow", steps: ["CO₂ 進入", "ATP 供能", "合成葡萄糖"], active: 1 }, duration: 3600 },
    { id: 5, caption: "暗反應把二氧化碳一個一個「固定」起來，慢慢組出葡萄糖 C₆H₁₂O₆。", ask: { prompt: "暗反應需要的能量貨幣來自哪裡？", options: ["土壤", "光反應產生的 ATP", "直接吸收陽光", "水蒸氣"], answer: 1, hint: "暗反應本身不吸光，它用的是光反應存下來的能量。" }, action: "jump", prop: { kind: "flow", steps: ["CO₂ 進入", "ATP 供能", "合成葡萄糖"], active: 2 }, duration: 3800 },
    { id: 6, caption: "把兩條線合起來就是總反應式，左右兩邊的原子數必須一樣多。", action: "point", prop: { kind: "balance", left: "6CO₂ + 6H₂O", right: "C₆H₁₂O₆ + 6O₂", tip: "反應條件寫在箭頭上：光能、葉綠體" }, duration: 3800 },
    { id: 7, caption: "對照一下：光合作用存能量、放出氧氣；呼吸作用釋放能量、消耗氧氣，兩者方向相反。", action: "cheer", prop: { kind: "text", text: "光合作用：CO₂＋H₂O → 養分＋O₂（存能量）", sub: "呼吸作用：養分＋O₂ → CO₂＋H₂O（釋能量）", tone: "ok" }, duration: 4000 },
  ],
  questions: [
...
export const CHEMICAL_CHANGE_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！冰塊化成水、鐵釘生鏽，這兩種「變了」其實是兩回事。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "冰塊融化成水：只是從固體變液體，成分都還是 H₂O，沒有新物質。", action: "point", prop: { kind: "flow", steps: ["冰（固體）", "水（液體）"], active: 1 }, duration: 3400 },
    { id: 3, caption: "這種只改變外形或狀態、物質本身不變的，叫做「物理變化」。", ask: { prompt: "把紙撕成兩半，是物理變化還是化學變化？", options: ["物理變化", "化學變化", "兩者都不是", "無法判斷"], answer: 0, hint: "撕碎的紙還是紙，成分沒變。" }, action: "think", prop: { kind: "text", text: "物理變化：形狀、狀態改變", sub: "物質本身不變，多半可以回復", tone: "ok" }, duration: 3600 },
    { id: 4, caption: "鐵釘生鏽就不一樣了：鐵（Fe）和空氣中的氧、水反應，生成「氧化鐵」這個新物質。", action: "walk", prop: { kind: "flow", steps: ["鐵 Fe", "＋ O₂、H₂O", "氧化鐵（鏽）"], active: 2 }, duration: 3600 },
    { id: 5, caption: "這種產生新物質、性質也跟著變的，叫做「化學變化」，通常無法輕易變回來。", ask: { prompt: "下列哪一個是化學變化？", options: ["冰塊融化", "鹽溶在水裡", "木材燃燒", "玻璃打破"], answer: 2, hint: "燃燒後產生灰燼和氣體，是新物質。" }, action: "jump", prop: { kind: "text", text: "化學變化：產生新物質", sub: "性質改變，多半不可逆", tone: "warn" }, duration: 3600 },
    { id: 6, caption: "化學變化常有的線索：顏色改變、產生氣體、沉澱、溫度明顯變化、發光。", action: "point", prop: { kind: "flow", steps: ["變色", "冒氣泡", "生成沉澱", "溫度變化"], active: 1 }, duration: 3600 },
    { id: 7, caption: "口訣：有沒有「新物質」？有→化學變化；沒有→物理變化。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const CELL_DIVISION_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！你從一個受精卵長到現在，靠的就是細胞不停分裂。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "第一步：細胞先生長，把裡面的東西都準備好，特別是「染色體」要複製一份。", action: "point", prop: { kind: "cycle", nodes: ["複製", "排隊", "分離", "分裂"], active: 0 }, duration: 3600 },
    { id: 3, caption: "第二步：複製好的染色體一對一對排在中間，準備平均分配。", ask: { prompt: "分裂前染色體要先做什麼？", options: ["先消失", "先複製成兩份", "先送出細胞", "不用做任何事"], answer: 1, hint: "這樣兩個子細胞才拿得到完整的一套。" }, action: "think", prop: { kind: "cycle", nodes: ["複製", "排隊", "分離", "分裂"], active: 1 }, duration: 3600 },
    { id: 4, caption: "第三步：成對的染色體被拉向兩端，一邊一份，誰也不多拿。", action: "walk", prop: { kind: "cycle", nodes: ["複製", "排隊", "分離", "分裂"], active: 2 }, duration: 3600 },
    { id: 5, caption: "第四步：細胞從中間一分為二，形成兩個一模一樣的子細胞。", ask: { prompt: "一個母細胞分裂後會產生幾個子細胞？", options: ["1 個", "2 個", "4 個", "很多個"], answer: 1, hint: "一次分裂就是「一分為二」。" }, action: "jump", prop: { kind: "cycle", nodes: ["複製", "排隊", "分離", "分裂"], active: 3 }, duration: 3600 },
    { id: 6, caption: "關鍵：子細胞的染色體數目和母細胞「相同」，所以複製這一步不能省。", action: "point", prop: { kind: "balance", left: "母細胞 2n", right: "子細胞 2n ＋ 2n", tip: "先複製再平分，數目才不會減半" }, duration: 3800 },
    { id: 7, caption: "口訣：先複製、再排隊、平均分、一分二。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const SPEED_RATE_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！甲跑 100 公尺花 20 秒，乙跑 100 公尺花 25 秒，誰比較快？", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "距離一樣時，花的時間越短就越快。所以甲比較快。", action: "point", prop: { kind: "bars", items: [{ label: "甲", value: 20 }, { label: "乙", value: 25 }], unit: "秒", active: 0 }, duration: 3400 },
    { id: 3, caption: "但如果距離不同，就不能只看時間了。這時要算「每秒跑幾公尺」——這就是速率。", ask: { prompt: "速率的計算公式是什麼？", options: ["距離 ÷ 時間", "時間 ÷ 距離", "距離 × 時間", "距離 ＋ 時間"], answer: 0, hint: "「每」單位時間走多少距離，所以用除法。" }, action: "think", prop: { kind: "text", text: "速率 = 距離 ÷ 時間", sub: "單位：公尺/秒（m/s）", tone: "ok" }, duration: 3600 },
    { id: 4, caption: "算算看：跑 100 公尺花 20 秒，速率 = 100 ÷ 20 = 5 公尺/秒。", action: "walk", prop: { kind: "balance", left: "100 m ÷ 20 s", right: "5 m/s", tip: "每秒前進 5 公尺" }, duration: 3600 },
    { id: 5, caption: "再看乙：100 ÷ 25 = 4 公尺/秒。5 > 4，用數字比就一清二楚。", ask: { prompt: "時速 90 公里開 2 小時，共走多遠？", options: ["45 公里", "180 公里", "92 公里", "90 公里"], answer: 1, hint: "距離 = 速率 × 時間 = 90 × 2。" }, action: "jump", prop: { kind: "bars", items: [{ label: "甲", value: 5 }, { label: "乙", value: 4 }], unit: "m/s", active: 0 }, duration: 3600 },
    { id: 6, caption: "最後一塊拼圖：「速度」比「速率」多了「方向」。只講快慢是速率，加上往哪裡才是速度。", action: "point", prop: { kind: "text", text: "速率：只有大小（5 m/s）", sub: "速度：大小 ＋ 方向（向東 5 m/s）", tone: "ok" }, duration: 3800 },
    { id: 7, caption: "口訣：速率＝距離÷時間；速度還要講方向。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const PLATE_TECTONICS_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！你腳下的地面其實是「拼圖」拼起來的，每一片都在慢慢移動。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "地球最外層的岩石圈破裂成好幾大塊，叫做「板塊」，下面是可以流動的地函。", action: "point", prop: { kind: "flow", steps: ["岩石圈", "板塊", "地函"], active: 1 }, duration: 3400 },
    { id: 3, caption: "板塊移動很慢，一年大概幾公分——跟指甲長大的速度差不多。", ask: { prompt: "板塊移動的速度大約如何？", options: ["每秒幾公里", "一年幾公分（很慢）", "完全不動", "一天幾公尺"], answer: 1, hint: "跟指甲長大的速度差不多。" }, action: "think", prop: { kind: "text", text: "板塊一年移動約幾公分", sub: "慢到感覺不出來，但千萬年後會造山", tone: "ok" }, duration: 3600 },
    { id: 4, caption: "兩個板塊互相擠壓（聚合），交界處會被擠皺隆起，形成山脈或海溝。", action: "walk", prop: { kind: "flow", steps: ["板塊擠壓", "地層隆起", "形成山脈"], active: 2 }, duration: 3600 },
    { id: 5, caption: "相反地，兩個板塊互相拉開（張裂），中間會裂開形成裂谷或新的海洋。", ask: { prompt: "兩個板塊互相「拉開」會形成什麼地形？", options: ["山脈", "裂谷或新海洋", "什麼都不會發生", "火山灰"], answer: 1, hint: "拉開就是讓中間空出來。" }, action: "jump", prop: { kind: "flow", steps: ["板塊張裂", "地殼裂開", "形成裂谷"], active: 2 }, duration: 3600 },
    { id: 6, caption: "板塊卡住時能量會一直累積，撐不住的瞬間一口氣釋放——那就是地震。", action: "point", prop: { kind: "flow", steps: ["板塊擠壓卡住", "能量累積", "瞬間斷裂", "發生地震"], active: 3 }, duration: 3800 },
    { id: 7, caption: "口訣：擠壓造山、張裂造谷、能量爆發就是地震。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const ENGLISH_TENSE_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！英文裡「吃飯」有兩種說法，用錯就會讓人誤會。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "I eat breakfast at 7. ——這是「習慣」：每天七點吃早餐，講的是常態。", action: "point", prop: { kind: "text", text: "I eat breakfast at 7.", sub: "現在簡單式：習慣、常態", tone: "ok" }, duration: 3600 },
    { id: 3, caption: "現在簡單式常出現這些關鍵字：every day、usually、always、on Sundays。", ask: { prompt: "「I ___ breakfast at 7 every day.」空格要用？", options: ["eat", "am eating", "ate", "eating"], answer: 0, hint: "every day 是習慣，用現在簡單式。" }, action: "think", prop: { kind: "text", text: "every day / usually / always", sub: "看到這些就是「習慣」→ 現在簡單式", tone: "ok" }, duration: 3800 },
    { id: 4, caption: "I am eating breakfast now. ——這是「正在做」：說話的這一刻正在吃。", action: "walk", prop: { kind: "text", text: "I am eating breakfast now.", sub: "現在進行式：此刻正在做", tone: "ok" }, duration: 3600 },
    { id: 5, caption: "現在進行式的公式很固定：be 動詞（am / is / are）＋ 動詞-ing。", ask: { prompt: "現在進行式的結構是？", options: ["主詞 + 原形動詞", "be 動詞 + V-ing", "主詞 + V-ed", "will + 原形動詞"], answer: 1, hint: "一定要有 be 動詞，動詞要加 -ing。" }, action: "jump", prop: { kind: "balance", left: "am / is / are", right: "＋ V-ing", tip: "I am / He is / They are" }, duration: 3800 },
    { id: 6, caption: "現在進行式常出現 now、right now、at the moment、Look!、Listen! 這些提示語。", action: "point", prop: { kind: "text", text: "now / right now / Look!", sub: "看到這些就是「此刻正在」→ 現在進行式", tone: "ok" }, duration: 3600 },
    { id: 7, caption: "口訣：習慣用簡單式（天天做），此刻用進行式（正在做）。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const CIRCLE_AREA_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！不管是大圓還是小圓，藏著一個永遠不變的數字——它就是 π。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "量量看：把圓的周長除以直徑，答案永遠大約是 3.14，這就是圓周率 π。", ask: { prompt: "圓周率 π 是怎麼算出來的？", options: ["周長 ÷ 直徑", "直徑 ÷ 周長", "半徑 × 2", "面積 ÷ 2"], answer: 0, hint: "周長大約是直徑的 3.14 倍。" }, action: "point", prop: { kind: "text", text: "π = 周長 ÷ 直徑 ≈ 3.14", sub: "不管圓多大，這個比值都一樣", tone: "ok" }, duration: 3800 },
    { id: 3, caption: "所以反過來：周長 = 直徑 × π。直徑 10 公分的圓，周長約 31.4 公分。", action: "walk", prop: { kind: "balance", left: "直徑 10 × 3.14", right: "周長 31.4", tip: "單位：公分" }, duration: 3600 },
    { id: 4, caption: "半徑是直徑的一半：直徑 10，半徑就是 5。面積公式用的是半徑。", ask: { prompt: "直徑 10 公分的圓，半徑是多少？", options: ["10 公分", "5 公分", "20 公分", "3.14 公分"], answer: 1, hint: "半徑是直徑的一半。" }, action: "think", prop: { kind: "shape", shape: "circle", base: 10, height: 5, label: "直徑 10 ‧ 半徑 5" }, duration: 3600 },
    { id: 5, caption: "圓面積 = 半徑 × 半徑 × π。半徑 5 的圓：5 × 5 × 3.14 = 78.5 平方公分。", action: "jump", prop: { kind: "text", text: "5 × 5 × 3.14 = 78.5", sub: "面積 = 半徑 × 半徑 × π", tone: "ok" }, duration: 3800 },
    { id: 6, caption: "為什麼？把圓切成很多小扇形再拼起來，會接近一個長方形：長是半周長、寬是半徑。", action: "point", prop: { kind: "flow", steps: ["切成扇形", "拼成長方形", "長 × 寬 = 面積"], active: 2 }, duration: 3600 },
    { id: 7, caption: "口訣：周長＝直徑×π，面積＝半徑×半徑×π。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
...
export const BA_BEI_LESSON: OnionLesson = {
  ...
  frames: [
    { id: 1, caption: "嗨！同一件事可以用三種句子講，今天學「把字句」和「被字句」。", action: "wave", prop: { kind: "none" }, duration: 3000 },
    { id: 2, caption: "基本句：我吃掉了蛋糕。主角是「我」，動作是「吃掉了」，對象是「蛋糕」。", action: "point", prop: { kind: "text", text: "我 吃掉了 蛋糕", sub: "主角 動作 對象", tone: "ok" }, duration: 3400 },
    { id: 3, caption: "把字句：把「對象」提前到動作前面——我把蛋糕吃掉了。主角還是「我」。", ask: { prompt: "「我把蛋糕吃掉了」的主角是誰？", options: ["蛋糕", "我", "吃", "沒有主角"], answer: 1, hint: "「我」是做出動作的人。" }, action: "think", prop: { kind: "text", text: "我把蛋糕吃掉了", sub: "把字句：主動者是主角", tone: "ok" }, duration: 3600 },
    { id: 4, caption: "被字句：改讓「對象」當主角——蛋糕被我吃掉了。", action: "walk", prop: { kind: "text", text: "蛋糕被我吃掉了", sub: "被字句：承受者是主角", tone: "ok" }, duration: 3400 },
    { id: 5, caption: "兩句意思一樣，只是「鏡頭」不同：把字句看主動者，被字句看承受者。", ask: { prompt: "「小狗咬破了鞋子」改成被字句是？", options: ["小狗把鞋子咬破了", "鞋子被小狗咬破了", "鞋子咬破了小狗", "小狗被鞋子咬破了"], answer: 1, hint: "被字句要讓「鞋子」當主角。" }, action: "jump", prop: { kind: "balance", left: "小狗把鞋子咬破了", right: "鞋子被小狗咬破了", tip: "意思相同，主角不同" }, duration: 3800 },
    { id: 6, caption: "轉換口訣：把字句＝A 把 B 怎麼了；被字句＝B 被 A 怎麼了。動詞永遠不改。", action: "point", prop: { kind: "flow", steps: ["A 把 B", "動作不變", "B 被 A"], active: 1 }, duration: 3600 },
    { id: 7, caption: "口訣：主動用把、被動用被；動詞不動，只換主角。準備闖關！", action: "cheer", prop: { kind: "none" }, duration: 2800 },
  ],
  questions: [
