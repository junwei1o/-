...
## 2026-09-20（深夜第三輪）「以上皆非」殘留 103 題歸零（老師追問變體題時發現）

老師問「變體題不是變態題，你清理掉了嗎」——複查時發現「以上皆非」並沒有真的清光：
**`scripts/gen/common.mjs` 的 `uniqueOptions` 在干擾池湊不滿四個選項時，會用
「以上皆非／無法判斷／與題目無關／條件不足」補位**，造成 103 題殘留
（數學 94、社會 5、國語 3、英語 1；正解都不是「以上皆非」，但它出現在選項裡）。

修法：刪掉補位清單。湊不滿四個選項就讓 `makeQuestion` 回 null、產生器換一組
數字重出——絕不用通用選項補位（與前端 `optionRandomizer` 同一條原則）。

重建後：總題數維持 5000（五科各 1000、年級各約 715）、含兜底選項 0 題、
變體題完好（三概念組合 579、雙概念組合 534、數學跨單元比較 40、知識點 1834）、
完全重複 0 題。體檢「沒有發現問題」，線上兩個題庫 chunk 都驗證過「以上皆非」0 次。

## 2026-09-21（凌晨）洋蔥學院「題庫劇場」＋課程擴充到 34 堂＋庫存互相組合新題型

### 題庫劇場（洋蔥學院第三分頁，`OnionBankTheater.tsx`）
- 洋蔥學院從兩個分頁變三個：「動畫課／**題庫劇場**／分數工坊」。
- 題庫劇場把 5000 題內建題庫與洋蔥動畫互相利用：每次開演依學生年級偏好
  （`buildChoiceDeck`）與選定科目抽題，洋蔥吉祥物全程演出（出題 point、
  答對 cheer、答錯 think），每題答完把 `explanation` 用字幕演出來。
  答錯可再試一次、不扣分；星級用 `gradeOnionLesson`，成績回寫教室最佳紀錄；
  各科目最佳星等存 localStorage（`hdmx_onion_theater_best_v1`）。
- 有測試（`OnionBankTheater.test.tsx`，含題庫動態載入的等待）。

### 動畫課 24 → 34 堂
- 國小 5：認識時刻、角度種類、植物的根莖葉、台灣的位置與地形、近義詞與反義詞
- 國中 5：電流與電路、力與平衡、台灣的氣候、提出公因式、英語被動語態
- **洋蔥學院首次收錄社會科**（台灣地形＋台灣氣候），五科齊。
- 測試：科目 4 → 5、堂數 ≥ 34。

### 庫存互相組合的新題型（`scripts/gen/matchingRelation.mjs` + `math.mjs`）
- **配對關係題 170 題**：把配對題庫 99 組的庫存轉成選擇題——正確選項是真實配對
  （l → r），干擾項是同組裡「接錯線」的配對。連連看與選擇題互相印證。
- **數學互相印證題 40 題**：同一單元出兩小題，問「甲的答案比乙多多少」——
  兩題都要算對才答得出。**選擇式題幹（下列哪一個…）會被過濾**，
  因為抽出來當小題後選項不見了會變殘題（實際踩過）。
...
## 2026-09-21 跨學科結合題（三科 120 ＋ 五科 80 ＝ 200 題）

### 做法：先寫「情境劇本」，不隨機拼貼
- `scripts/gen/crossSubject.mjs`：**每個劇本都是一個真實情境**，情境本身同時需要 3 或 5 個科目才解得開，例如：
  - 三科：營養午餐的在地食材（數學算價差＋社會產地運輸＋自然食物里程）、水庫水位與限水、地震與防災、太陽能與電費、回收率、降雨機率的讀法、植物生長紀錄、老街預算、班級調查圖表、運動成績單位、夜市消費英文、垃圾分類標語、雙語氣象報告、動物名稱中英對照、摘要字數。
  - 五科：雙語營養午餐海報、颱風停班停課公告、校外教學行前通知、校園節能減碳週、臺灣水果外銷、世界地球日、園遊會攤位、河川水質調查、早餐怎麼選。
- **每個劇本的資料由程式隨機產生、答案由程式算出**（數字一定對），同一劇本換參數就長出不同題目。
- 選項一律是「**數據 ＋ 學科判斷**」的組合：學生必須同時算對、也判斷對。干擾項都是學生真的會犯的錯（算錯、把因果顛倒、用錯字或英文）。
- 每題 `knowledge` 與科目一對一（有幾個科目就有幾個知識點），並新增 `subjectCombination` 欄位記錄結合了哪幾科。
- **主科輪換**：跨科題在資料庫只能掛一個科目，若一律掛第一個會全擠在數學（曾 184/200），改成在情境科目間輪流掛 → 英語 25／數學 51／社會 49／自然 45／國語 30。
- 整合進 `expand-question-bank.mjs`，並計入各科 1000 題的額度，所以**全站總數仍為 5000 題**。
- `build-runtime-bank.mjs` 的 KEEP 加入 `subjectCombination`（否則打包時會把跨科資訊丟掉）。
- 體檢新增跨科題區塊（數量、三科/五科分佈、科目與知識點是否一對一）；長度上限分兩類：一般題 60 字、跨科題 110 字（跨科選項要同時承載數據與各科判斷，硬砍會讓題意不清）。
- 目前體檢：**沒有發現問題**。

## 2026-09-21（下午）洋蔥學院補齊高中 55 堂，達成 200 堂目標

### 課程規模：145 → 200 堂
| 學段 | 堂數 | 題數 | 科目 |
|---|---|---|---|
| 國小 | 70 | 350 | 數學、國語、自然、社會、英語 |
| 國中 | 65 | 325 | 數學、自然、社會、國語、英語 |
| 高中 | 65 | 325 | 數學 10、物理 8、化學 8、生物 8、地球科學 6、歷史 6、地理 6、公民 5、國文 4、英文 4 |
| 合計 | **200** | **1000** | 14 科 |

- 步驟標籤 **1418 / 1418 幀**（每一幀都有「步驟 N：具體動作」）；內容體檢 **0 問題**。
- 高中 65 堂與 `docs/onion-200-plan.md` 的課表**逐堂核對一致**（無遺漏、無多餘）。

### 新增的工程資產
- **`docs/onion-authoring-spec.md`**：撰寫一堂動畫課的完整規格（檔案結構、7 幀分鏡規則、
  `OnionProp` 每個 kind 的欄位、圖解一致性硬規則、題目規則、驗證指令）。
  並行產課時所有寫手讀同一份，風格與驗收標準才一致。
- **`docs/onion-200-plan.md`** 的批次改成「一檔一批」（原本地科＋歷史、地理＋公民、
  國文＋英文兩兩共用一個檔案 → 兩位寫手會搶同一個檔）。
- 體檢新增兩項檢查：**題目 id 全站唯一**、**id 需以自己課程 id 開頭**。

### 修掉的實際問題
- **題目 id 撞號**：1000 題裡有 85 個重複（例：角的分類與大氣的結構都用 `at1`–`at5`）。
  App 目前用「題目索引」追蹤作答所以還沒出錯，但題目 id 是天然的作答紀錄鍵，
  一旦改用它存進度就會互相污染。已把 1000 個題目 id 全部正規化為 `<課程 id>-<序號>`
  （一次性腳本 `scripts/_normalize-question-ids.py`，執行後已刪除），並把檢查併入 `qc:onion`。
- **國文術語**：〈登鸛雀樓〉那堂原本寫「句句對仗」，但絕句並不要求對仗
  （對仗是律詩的講究），與下一幀自己的說明矛盾 → 改為「這兩句正好對仗」。
- **體檢顯示瑕疵**：步驟標籤分母原本寫死「課數 ×7」，核心課程有 10 幀的課會印出
  分子大於分母（1033/1015）→ 改用實際分幀總數。
- 兩個過時測試（寫死「5 個科目、2 個學段」）改寫成新合約並加強：列出全部 14 科、
  三個學段都要達標、選課頁要能篩出「高中」分頁。

### 驗證
- `npm run qc:onion`：0 問題（含題目 id 唯一性）。
- `npx tsc --noEmit`：0 錯誤。
- 全套測試 **1141 項全過**（lib 291／game 393／components 191／pages 161／server 105）。
- 抽樣人工驗算：高中物理 40 題、化學 40 題的數值**逐題重算，全部正確**；
  歷史年代、生物遺傳比例與能量塔 10%、地科 P/S 波特性均核對無誤。

### 注意事項
- 並行產課期間多位寫手曾因平台額度限制（429）中斷，額度重置後以 9 位寫手平行補齊；
  本輪 9 批全部一次交件、0 問題。
